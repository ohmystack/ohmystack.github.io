...
* `Pillar` 存放 key-value 变量。存放在 Master 端，由 Master 编译好后，下发给 Minion。所以，可以存放密码之类的涉密的、需要统一配置的变量。
...
如果不配置这一项，Minion 会自动通过 DNS 尝试连接 `salt` 这个 hostname 的服务器。
...
> 3. /etc/hosts (%WINDIR%\system32\drivers\etc\hosts on Windows hosts) 中定义的> 任何映射 127.0.0.0/8 的主机名做ID值
> 
...
> 所有 module 列表： http://docs.saltstack.com/en/latest/ref/modules/all/
...
至此，Salt 入门完成了。你已经学会用 salt 自带的 modules，对你选定范围对 minion 执行命令了。
